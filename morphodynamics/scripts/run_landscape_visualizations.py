...
class Visualisations():
    ...
    def gradient_colored(self, plot_every):
        ...
        UArray_plot = self.UArray[3*resolution:23*resolution, :]
        UArray_plot = UArray_plot[:, 3*resolution:23*resolution]
        xg = xg[3*resolution:23*resolution, :]
        xg = xg[:, 3*resolution:23*resolution]
        yg = yg[3*resolution:23*resolution, :]
        yg = yg[:, 3*resolution:23*resolution]

        Gx, Gy = np.gradient(UArray_plot, 20./1000, 20./1000) # gradients with respect to x and y
        G = (Gx**2+Gy**2)**.5  # gradient magnitude
        G[np.isinf(G)] = np.nan

        mesh = mlab.mesh(xg, yg, 10*UArray_plot, scalars = G, figure = figU, colormap = 'jet', opacity = 1, vmin = 0, vmax = 0.2)
        ...
        #mlab.show() # uncomment this to view in interactive mode
        #plt.show()
